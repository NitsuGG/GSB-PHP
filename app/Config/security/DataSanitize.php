...
class DataSanitize
{
    ...
    {
        if (!isset(($_SESSION['csrf_token']))) {
            log_message('error', "Token introuvable, l'input n'a pas pu être crée");
            throw new Exception("There is no csrf token stored int session. Can't create input.");
        }
    ...
    public function csrf_verification(String $session_csrf_token): void
    {
        if (isset($session_csrf_token) && isset($_GET['csrf_token'])) {
            if ($_GET['csrf_token'] !== $session_csrf_token) {
                log_message('alert', "Token client et token serveur different !");
